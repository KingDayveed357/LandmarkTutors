...
if (isset($_POST['change_pass'])){
 ...
 $sql = $conn->query("SELECT email  AND password FROM student WHERE email='$email' AND password='$password' ");
 if($sql->num_rows>0){
     if($newpass === $cnewpass && (strlen($_POST['newpass']) >=7)){
 $sql = $conn->query("UPDATE student SET password='$newpass' WHERE email='$email' AND password='$password' ");
...
//  echo 'You have successfully updated your password ';
  ...
  Password must be 7 characters or above 
 <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
 </div>' ; 
}else {
  $status = '<div class="alert alert-danger alert-dismissible" role="alert">
