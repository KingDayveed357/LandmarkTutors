            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <?php 
               ...
               $studentId = $_SESSION['studentId'];
							$sql= $conn->query(" SELECT * FROM `my_courses` WHERE courseId='$videoId' AND studentId='$studentId'");
							if($sql->num_rows>0){
							while($row=$sql->fetch_assoc()){  ?>  
              <div class="row mb-5">
              <div class="col-lg-12">
              <div class="card p-3 px-lg-4">
              <iframe width="auto" class="" height="500" src="<?php echo $row['course_content'] ?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
              <h4 class="mt-4"><?php echo $row['course_name']; ?></h4>
              <h6><i class='bx bx-calendar me-2 my-auto'></i><?php echo $row['date']; ?></h6>
              <hr/>
              <div class="d-flex mt-3">
                          <div class="flex-shrink-0 me-3">
                            <div class="avatar ">
                            <?php if (empty($row['tutorProfile'])) {
     ?>    <img src="../assets/img/avatars/instructor-profile.jpg" alt  class="w-px-40 w-100  h-100  rounded-circle"/>
     <?php
  }else {
      ?> 
    
    <img src="<?php echo $row['tutorProfile']; ?>" alt class="w-px-40 w-100  h-100  rounded-circle" />  
      <?php
  }
  ?>
                          ...
                          <div >
                            <h4 class="fw-bold d-block"><?php echo $row['tutor_name']; ?></h4>
                            <h6 class="text-muted"  style="margin-top:-15px">Instructor</h6>
                          </div>
                        </div>
                        <div>
                        <!-- <button class="btn btn-outline-primary">View Profile</button> -->
                        </div>
                        <p class="mt-2"><?php echo $row['course_description']; ?></p>
            </div>
              </div>
              </div>
